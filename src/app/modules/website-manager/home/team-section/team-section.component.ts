...
export class TeamSectionComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.getHomeTeamSectionText();
  }

  initializeFormGroup() {
    this.homeTeamSectionForm = this.formBuilder.group({
      id: null,
  ...
  onSubmitForm(): void {
    ...
    this.submission$ = this._dataService.post(this.homeTeamSectionForm.value, 'website/home/team-section')
      .subscribe({
        next: (response) => {
  ...
  getHomeTeamSectionText() {
    this._commonService.onBufferEvent.emit(true);
    this.subscription$ = this._dataService.getJson('website/home/team-section', '')
      .subscribe({
        next: (response) => {
          this._commonService.onBufferEvent.emit(false);
          if (response.code == 200) {
            this.homeTeamSectionForm.patchValue(response.data);
          }
        },
        error: (error) => {
          console.error(error);
